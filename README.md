...
With version **0.2.14**, the smart thermostat (TRV) is introduced and optimizations were implemented that improves the interaction with the Node-RED dashboard: 
- **msg.topic** now contains the name of a device, so that several devices can be separated in a **dashboard chart**. You are also free to configure **msg.topic** via the property **name**.
- The output of **boolean services** has been aligned with the input if the corresponding **state** is set. The Node-RED **dashboard switch** can now be connected directly to boolean services to toggle the switch state, such as the camera or the plug.
